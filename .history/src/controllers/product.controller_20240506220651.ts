...
const topProductController = async (req: Request, res: Response) => {
  ...
  const searchParams = { top, quantity };

  // Kiểm tra xem top và quantity có phải là chuỗi không
  if (typeof searchParams.top === 'string') {
    searchParams.top = [searchParams.top];
  }
  if (typeof searchParams.quantity === 'string') {
    searchParams.quantity = parseInt(searchParams.quantity as string, 10); // Chuyển đổi chuỗi thành số nguyên
  }

  // Mặc định cho quantity là 6 nếu không được cung cấp
  if (typeof searchParams.quantity !== 'number' || isNaN(searchParams.quantity)) {
    searchParams.quantity = 6;
  }

  // Gọi topProductService với các tham số phù hợp
  const returnData = await topProductService(searchParams.top[0], searchParams.quantity);
  return res.status(returnData.status).json(returnData.data);
}
