...
import { createOderService, getOrderByIdService, getOrderUserService } from "../services/order.service";
...
const cancelOrderController = async (req: Request, res: Response) => {
  ...
  const returnData = await cancelOrderController(req.user["id"], req.body.orderId);
  return res.status(returnData.status).json(returnData.data);
};
