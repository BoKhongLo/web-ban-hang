...
const validateVnpayController: RequestHandler = async (req: Request, res: Response) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    }
    const ipAddr = req.headers['x-forwarded-for'];
    const returnData = await vali(req.user["id"], req.query.orderId ?? req.query.orderId[0] , ipAddr);
    return res.status(returnData.status).json(returnData.data);
};
export {
    createVnpayController,
    validateVnpayController
}
