...
import {getUserByIdService, editProfileService, addHeartProductService} from "../services/user.service";
...
const addHeartProductController = async (req : Request, res : Response) => {
  ...
  const returnData = await addHeartProductService(req.body.productId);
  return res.status(returnData.status).json(returnData.data);
};
