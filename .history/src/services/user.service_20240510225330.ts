...
export async function editRoleService(dto: EditRoleDto) {
    try {
        const userCheck = await UserModel.findOne({ id: dto.adminId }).select('+hash');
        if (userCheck.role.includes("ADMIN")) {
            return { data: { error: "The user is not permission" }, status: 401 };
        }
        const userEdit = await UserModel.findOne({ id: dto.userId }).select('+hash');
        if (!userEdit) {
            return { data : {error: "User is not exist"}, status: 401 };
        }
        userEdit.role = new Types.Array<string>();
        userEdit.role.push(...dto.role)
        await userEdit.save();
        let dataReturn = userEdit.toJSON();
        delete dataReturn.refresh_token;
        delete dataReturn.hash;
        return { data :dataReturn, status: 200 };
    }
...
export async function getAllUserService(adminId: string) {
    try {
        const userCheck = await UserModel.findOne({ id: adminId }).select('+hash');
        if (userCheck.role.includes("ADMIN")) {
            return { data: { error: "The user is not permission" }, status: 401 };
        }
        let listUsers- await UserModel.find().select("hash")
        let dataReturn = user.toJSON();
        delete dataReturn.refresh_token;
        delete dataReturn.hash;
        return { data : dataReturn, status: 200 };
