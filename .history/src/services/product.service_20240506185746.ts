...
export async function searchProductService(params: any) {
  try {
    ...
    if (params.brand.length > 0) filter['detail.company'] = { $in: params.brand };
    if (params.color.length > 0) filter['color'] = { $in: params.color };
    if (params.size.length > 0) filter['size'] = { $in: params.size };
    if (params.category.length > 0) {
      const categoryRegex = params.category.map(cat => new RegExp(cat, 'i')); // Tạo một mảng các biểu thức chính quy từ các giá trị trong mảng category
      filter['productType'] = { $in: categoryRegex }; // Sử dụng $in để lọc bất kỳ giá trị nào trùng khớp với bất kỳ biểu thức chính quy nào trong mảng
    }
    if (params.title.length > 0) filter['productName'] = { $regex: params.title[0], $options: 'i' };
