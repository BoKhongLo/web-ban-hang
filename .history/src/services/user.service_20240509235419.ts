...
export async function editRoleService(role: string[], userId: string, adminId: string) {
    try {
        ...
        const userEdit = await UserModel.findOne({ id: userId }).select('+hash');
        if (userCheck.role.includes("ADMIN") {
            return { data: { error: "The user is not permission" }, status: 401 };
        }
        userEdit.role = new Types.Array<string>();
        userEdit.role.push(...newData)
        await userEdit.save();
