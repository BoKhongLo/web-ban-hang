...
export async function topProductService(top: string, quantity: number = 6) {
  try {
    if (top === "hot-sales") { // Sử dụng '===' thay vì '=' để so sánh giá trị
      productList = await ProductModel.find().sort({ byCount: -1 }).limit(quantity); // Sắp xếp theo số lượt mua giảm dần và giới hạn số lượng sản phẩm lấy
    }
    else if (top === "new-arrivals") {
      productList = await ProductModel.find().sort({ createdAt: -1 }).limit(quantity); // Sắp xếp theo ngày tạo mới giảm dần và giới hạn số lượng sản phẩm lấy
    }
