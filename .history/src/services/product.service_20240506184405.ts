...
export async function searchProductService(params: any) {
  try {
    ...
    if (params..length > 0) filter['detail.tags'] = { $in: params.tags };
    // if (params.brand) filter['detail.company'] = { $in: params.brand };
    // if (params.color) filter['color'] = { $in: params.color };
    // if (params.size) filter['size'] = { $in: params.size };
    // if (params.category) filter['productType'] = params.category;
