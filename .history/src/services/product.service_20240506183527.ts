...
export async function searchContentService(params: any) {
  try {
    ...
    if (params.tags) filter['detail.tags'] = { $in: tags };
    if (params.brand) filter['brand'] = { $in: brand };
    if (params.color) filter['color'] = { $in: color };
    if (params.size) filter['size'] = { $in: size };
    if (params.category) filter['productType.main'] = category;
